  ...
  function handleTableChange(
    ...
    if (sorter && isFunction(sortFn)) {
      ...
      searchState.sortInfo = {
        fields: [sortInfo.field],
        orders: [sortInfo.order],
      };

      // searchState will be used in fetch
  ...
  async function fetch(opt?: FetchParams) {
    ...
    try {
      ...
      if ((isBoolean(pagination) && !pagination) || isBoolean(getPaginationInfo)) {
        pageParams = { current: 1, pageSize: 100 };
      } else {
      ...
      let params: Recordable = {
        page: pageParams,
        search: merge(searchInfo, opt?.searchInfo),
        sorter: merge(defSort, sortInfo, opt?.sortInfo),
        filter: { fields: [], values: [] },
      };

      const tmpFilter = { ...filterInfo };
      const filterParams = merge(tmpFilter, opt?.filterInfo);
      const filterKeys = Object.keys(filterParams);
      if (filterKeys.length > 0) {
        for (const key of filterKeys) {
          if (filterParams[key] && filterParams[key] != '') {
            params.filter.fields.push(key);
            params.filter.values.push(filterParams[key]);
          }
        }
      }

      if (!params.search.value || params.search.value == null) {
        params.search = null;
      }
      if (params.sorter.fields == null || params.sorter.fields[0] == null) {
        params.sorter = null;
      }

      if (
        params.filter.fields == null ||
        params.filter.fields.length == 0 ||
        params.filter.values == null ||
        params.filter.values.length == 0
      ) {
        params.filter = null;
      }
